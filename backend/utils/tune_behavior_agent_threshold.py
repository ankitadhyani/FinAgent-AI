...
def evaluate_threshold(df, customer_profiles, threshold):
    tp = fp = tn = fn = 0

    for _, row in df.iterrows():
        txn = row.to_dict()
        score = analyze_behavior(txn, customer_profiles)["behavior_score"]
...
if __name__ == "__main__":
    ...
    results = [
        evaluate_threshold(sample_df, customer_profiles, i / 100)
