...
def build_tuning_sample(df):
    ...
    fraud = fraud_df.sample(n=min(FRAUD_SAMPLE_SIZE, len(fraud_df)), random_state=RANDOM_SEED)
    normal = normal_df.sample(n=min(NORMAL_SAMPLE_SIZE, len(normal_df)), random_state=RANDOM_SEED)
...
def evaluate_threshold(df, threshold):
    ...
    for _, row in df.iterrows():
        txn = row.to_dict()
        score = analyze_location(txn)["location_score"]
...
if __name__ == "__main__":
    ...
    results = [
        evaluate_threshold(sample_df, i / 100)
        for i in range(10, 81, 5)
    ]

    results = sorted(results, key=lambda x: x["f1"], reverse=True)

    print("\n===== LOCATION THRESHOLD TUNING =====")
    for result in results:
        print(result)

    print("\n===== BEST LOCATION THRESHOLD =====")
    print(results[0])